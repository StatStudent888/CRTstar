...
nE = 200                                      # External sample sizes (multiple values for comparison)
N = 200                                       # Test sample sizes
N_un = 200                                    # Unlabeled high-heterogeneity data size (number of unlabeled samples)
...
# c, cE = 0, np.sqrt(np.exp(0.04/(nE*k_E))-1) # Under the null hypothesis, take cE by order under different strengths of conditional dependence.
...
# beta_E = beta
beta_E = [0] * 1 + [0.3] * s + [0] * (p - s - 1)   # External Beta

np.random.seed(24)
gamma_0 = np.random.normal(0, 1, s)
gamma = np.zeros(p)                                # Initialize gamma
gamma[int(s/2):(int(s/2)+s)] = gamma_0
...
# gammaE = gamma
gammaE = np.zeros(p)     
gammaE[int(s/2):(int(s/2)+s)] = gamma_0E           # External gamma
...
aU = np.zeros((K, p))
A_real = np.arange(10)    
AE_real = np.arange(2, 12)

beta_0 = [0] * 1 + [0.3] * (s - 1) + [0] * (p - s)

A_only = set(A_real) - set(AE_real) 
AE_only = set(AE_real) - set(A_real)
both = set(A_real) & set(AE_real)   
...
for i in range(K):                                 # Unlabel Beta
    if i in A_only:
        ...
        Hk = np.random.choice(range(p), Hk_num-1, replace=False)
        aU[i] = beta_0.copy()
        for j in Hk:
            aU[i, j] -= 0.3
    else:
        Hk = np.random.choice(range(p), 5 * s, replace=False)
        aU[i] = beta_0.copy()
        for j in Hk:
            aU[i, j] -= 5  

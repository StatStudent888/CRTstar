    ...
    if nu == 0 :
        ...
        for i in range(K):
            if i in union:
                ...
                u_un_num += 1
        XU_all_eps = np.concatenate([XU_eps.ravel(), XU_un_eps.ravel()])
    data = {'z': z ,'X': X ,'Y': Y ,'zE': zE ,'XE': XE ,'YE': YE ,'zU': zU_l ,'XU': XU_l,'X_eps': X_eps, 'XE_eps': XE_eps, 'XU_all_eps': XU_all_eps}
    return data

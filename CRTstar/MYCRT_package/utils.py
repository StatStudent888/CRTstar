...
def Split_the_data(seed, CRT, data, k_in, k_E):
    ...
    ne , _ = zE0.shape
    if CRT == 'out':
        X_train, X, _, Y, z_train, z = train_test_split(X0, Y0, z0, test_size=k_in, random_state=seed) # seed=42 for simulation studies
        if ne == 0:
            XE = YE = XE_train = np.empty((0,))
            zE = zE_train = np.empty((0, p))
        else:
            XE_train, XE, _, YE, zE_train, zE = train_test_split(XE0, YE0, zE0, test_size=k_E, random_state=seed) # seed=42 for simulation studies
